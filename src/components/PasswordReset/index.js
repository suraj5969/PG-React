...
import passwordResetAPI from '../../apis/admin/passwordResetAPI';
...
function PasswordReset() {
    ...
    const PasswordResetAuth = async () => {
        ...
        const apiResponse = await passwordResetAPI(user_id, values.currentPassword, values.password,);
        // console.log(apiResponse);
        if (apiResponse.status === 203) {
            setValues({ ...values, isSuccessful: false, isCurrentPasswordCorrect: false, });
            return;
        }
        if (apiResponse.status === 200) {
            ...
            setValues({ ...values, isSuccessful: true });
            ...
            setValues({ ...values, isSuccessful: false, password: '', confirmPassword: '' });
        }
    }
    ...
    const setCurrentPassword = (key) => {
        setValues({ ...values, currentPassword: key, isCurrentPasswordCorrect: true });
    }
            ...
            <Box className={styles.innerSM}>
                ...
                <Box className={styles.innerinner}>
                    ...
                    <Stack className={styles.form} spacing={2}>
                        <FormControl
                            className={
                                values.isCurrentPasswordCorrect
                                    ? styles.inputBox
                                    : styles.inputBoxErr
                            }
                        >
                        ...
                        </FormControl>
                        {
                            values.isCurrentPasswordCorrect
                                ? ''
                                : <p className={styles.helpText}>Current password does not match.</p>
                        }
                        <FormControl
                            className={
